...
async def get_portal_id(client: ZohoClient):
    ...
    if portals_list and isinstance(portals_list, list) and portals_list[0].get("id_string"):
...
async def get_task_utilisation(client: ZohoClient, project_id: str):
    ...
    utilisation = {m.get("name", "Unknown"): 0 for m in members}
